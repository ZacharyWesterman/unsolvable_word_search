...
bool wordSearch::load(std::istream &input) {
	clear();
	...
	for (int i = 1; i < letterGrid.length(); ++i) {
		if (letterGrid[i].length() > len) {
			len = letterGrid[i].length();
		}
	}
...
void wordSearch::randomize(int rows, int cols) {
	clear();
	...
	for (int y = 0; y < dataHeight; ++y) {
		for (int x = 0; x < dataWidth; ++x) {
			data[y * dataWidth + x] = 'A' + (generator() % 26);
			highlighted[y * dataWidth + x] = 0;
		}
	}
...
int wordSearch::find(const z::core::string<> &text, int color) {
	matches.clear();
	if (!text.length()) {
		return 0; // null string never matches any
	}

	const int directions[8] = {
		-dataWidth - 1, -1, dataWidth - 1, dataWidth, -dataWidth, -dataWidth + 1, 1, dataWidth + 1,
	};
	...
	for (int i = 0; i < dataWidth * dataHeight; ++i) {
		if (z::core::toUpper(data[i]) != z::core::toUpper(text[0])) {
			continue;
		}
...
void wordSearch::setHighlight(int pos, int dir, int len, int color) {
	for (int i = 0; i < len; ++i) {
		if ((pos < 0) || (pos >= dataWidth * dataHeight)) {
			break;
		}
...
int wordSearch::scan(const z::util::dictionary &dict, int minLength, int maxLength, int color) {
	...
	const int directions[8] = {
		-dataWidth - 1, -1, dataWidth - 1, dataWidth, dataWidth + 1, 1, -dataWidth + 1, -dataWidth,
	};
	...
	for (int length = minLength; length <= maxLength; ++length) {
		for (int y = 0; y < dataHeight; ++y) {
			for (int x = 0; x < dataWidth; ++x) {
				...
				if (!dict.narrow(&rangeMain, data[pos])) {
					continue;
				}
				...
				for (int j = 0; j < 8; ++j) // go in circle
				{
					...
					if ((xend >= dataWidth) || (xend < 0)) {
						continue;
					}
					if ((yend >= dataHeight) || (yend < 0)) {
						continue;
					}
					...
					for (int i = 1; i < length; ++i) {
						xend += dirs[j][0];
						yend += dirs[j][1];
						if (!dict.narrow(&range, data[yend * dataWidth + xend])) {
							break;
						}
					}
					if (range.exhausted || !range.isWord) {
						continue; // not a word that matches the minimum, look in next direction.
					}
